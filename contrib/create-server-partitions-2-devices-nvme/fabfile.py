...
expected bare metal server with two *empty* HDDs /dev/nvme0n1 and /dev/nvme1n1 in LiveCD mode
...
GPT:
/dev/nvme0n1p1, /dev/nvme1n1p1 -    2014s - bios_grub partitions
/dev/nvme0n1p2, /dev/nvme1n1p2 -    1 GiB - /dev/md0, /boot
/dev/nvme0n1p3, /dev/nvme1n1p3 -   32 GiB - /dev/md1, LVM
/dev/nvme0n1p4, /dev/nvme1n1p4 - all rest - for zfs zpool
...
class Parted(object):
    # blockdev --getsize64 /dev/nvme0n1 returns size in bytes
    # blockdev --getsize /dev/nvme0n1 returns size in sectors
...
def is_disks_has_no_partitions():
    ...
    devices -= set(['loop0'])
    return devices == set(['nvme0n1', 'nvme1n1'])
...
def create_partitions():
    ...
    run("parted -s /dev/nvme0n1 -- set 1 bios_grub on")
    run("parted -s /dev/nvme1n1 -- set 1 bios_grub on")

    run("parted -s /dev/nvme0n1 -- set 2 raid on")
    run("parted -s /dev/nvme1n1 -- set 2 raid on")

    run("parted -s /dev/nvme0n1 -- set 3 raid on")
    run("parted -s /dev/nvme1n1 -- set 3 raid on")

    time.sleep( 1 )

    run("mdadm --create /dev/md0 --level=mirror --raid-devices=2 /dev/nvme0n1p2 /dev/nvme1n1p2 --metadata=1.2")
    run("mdadm --create /dev/md1 --level=mirror --raid-devices=2 /dev/nvme0n1p3 /dev/nvme1n1p3 --metadata=1.2")
